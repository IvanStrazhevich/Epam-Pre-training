package by.epam.preTraining.ivanStrazhevich.tasks.task2.spellNumber;
...
public class SpellNumber {
    ...
    public static StringBuffer spellNumber(Integer number) {
        ...
        result.append("Введите число в диапазоне от 0 до 999 включительно");
        if (number >= 0 && number < 1000) {
            int hundreds = number / 100;
            int dozens = number % 100 / 10;
            int digits = number % 10;

            result.replace(0, result.length(), "Число в виде строки: ");
            if (number == 0) {
                result.append("ноль");
            } else if (number < 10) {
